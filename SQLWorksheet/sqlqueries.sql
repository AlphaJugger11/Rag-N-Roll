...
show models;
-- SELECT * FROM SNOWFLAKE.CORTEX.PUBLIC_FUNCTIONS();
-- SELECT * 
-- FROM INFORMATION_SCHEMA.FUNCTIONS 
-- WHERE FUNCTION_NAME LIKE '%CORTEX%';
SELECT SNOWFLAKE.CORTEX.EMBED_TEXT_768('snowflake-arctic-embed-m', 'example text') AS embedding;
SELECT SNOWFLAKE.CORTEX.COMPLETE('mistral-large', 'example prompt') AS response;
SELECT FUNCTION_NAME, FUNCTION_SIGNATURE
FROM INFORMATION_SCHEMA.FUNCTIONS
WHERE FUNCTION_SCHEMA = 'CORTEX';

SELECT SNOWFLAKE.CORTEX.COMPLETE_MISTRAL('example prompt') AS response;
SHOW FUNCTIONS IN SCHEMA SNOWFLAKE.CORTEX;

-- DESCRIBE FUNCTION SNOWFLAKE.CORTEX.COMPLETE;
SHOW FUNCTIONS IN SCHEMA CORTEX;
